...
if ! [ -f "$FILE" ]; then
  ...
  --password $COUCHBASE_ADMINISTRATOR_PASSWORD \
  --bucket $COUCHBASE_BUCKET \
  --create-collection my_scope.collection1 \
  --max-ttl 0
  ...
  #-d 'CREATE PRIMARY INDEX prm ON `tarun`.`my_scope`.`collection1`'

  /opt/couchbase/bin/curl -u Administrator:password http://localhost:8093/query/service -H 'Content-Type: application/json' -d '{"statement":"CREATE PRIMARY INDEX ON tarun.my_scope.collection1"}'
  sleep 2s
  ...
  -d "file:///opt/couchbase/init/sample-data.json" -b 'tarun' -g %projectId% --scope-collection-exp 'my_scope.collection1'
